...
ManipulationPlanningInterface::ManipulationPlanningInterface(const ros::NodeHandle& node_handle,
  ...
  XmlRpc::XmlRpcValue execute_manipulation_planning_srv_id;
  if (!getParam(nh_, pnh_, "execute_manipulation_planning_srv_id", execute_manipulation_planning_srv_id)) {
    return;
  }
  execute_manipulation_planning_srv_ = nh_.advertiseService(
      execute_manipulation_planning_srv_id, &ManipulationPlanningInterface::executeManipulationPlanningSrvCb, this);
...
auto ManipulationPlanningInterface::createRobot() -> bool {
  XmlRpc::XmlRpcValue robot_name;
  if (!getParam(nh_, pnh_, "robot_name", robot_name)) {
    return false;
  }
...
auto ManipulationPlanningInterface::setBound() -> bool {
  XmlRpc::XmlRpcValue x_lower;
  if (!getParam(nh_, pnh_, "x_lower", x_lower)) {
    return false;
  }
...
auto ManipulationPlanningInterface::createObstacle() -> bool {
  XmlRpc::XmlRpcValue obstacle_name;
  if (!getParam(nh_, pnh_, "obstacle_name", obstacle_name)) {
    return false;
  }
...
auto ManipulationPlanningInterface::executeManipulationPlanningSrvCb(roport::ExecutePathPlanning::Request& req,
                                                                     roport::ExecutePathPlanning::Response& resp)
    -> bool {
  if (is_initial_location_set_ && is_initial_state_set_) {
    ROS_INFO("Path planning started ...");
  } else {
  ...
  //      return false;
  //    }
  //    ROS_INFO_STREAM("Curr: " << q_current_[0] << " " << q_current_[1] << " " << q_current_[2] << " " <<
  //    q_current_[3]); ROS_INFO_STREAM("Goal: " << q_goal_[0] << " " << q_goal_[1] << " " << q_goal_[2] << " " <<
  //    q_goal_[3]); path_planning_solver_->initConfig(std::make_shared<Configuration_t>(q_current_));
  //    path_planning_solver_->solve();
  //
  //    // Get the last path from paths, which is an optimized one
  //    PathPtr_t optimized_path(path_planning_solver_->paths().back());
  ...
  //
  //    int intervals = static_cast<int>(path_length / time_step_);
  //    ROS_INFO("Planning finished. Path time duration: %f, waypoints #%i, Sending command ...", path_length,
  //    intervals);
  //
  //    std::vector<sensor_msgs::JointState> joint_states;
  //    std::vector<geometry_msgs::Twist> vel_cmd;
  ...
  //  // Necessary for using the last goal
  //  path_planning_solver_->resetGoalConfigs();
  ROS_INFO("Manipulation execution finished.");
  return true;
}

auto ManipulationPlanningInterface::setJointConfig(const sensor_msgs::JointState& msg,
                                                   hpp_core::Configuration_t& config,
                                                   const bool& update_names) -> size_t {
  if (update_names) {
    joint_names_.clear();
...
auto ManipulationPlanningInterface::setLocationConfig(const geometry_msgs::Pose& msg,
                                                      const int& type,
...
void ManipulationPlanningInterface::currentJointConfigCb(const sensor_msgs::JointState::ConstPtr& msg) {
  if (!is_initial_state_set_) {
    setJointConfig(*msg, q_current_, true);
    is_initial_state_set_ = true;
  } else {
...
void ManipulationPlanningInterface::currentLocationCb(const nav_msgs::Odometry::ConstPtr& msg) {
  setLocationConfig(msg->pose.pose, 0, q_current_);
  is_initial_location_set_ = true;
}
}  // namespace roport
