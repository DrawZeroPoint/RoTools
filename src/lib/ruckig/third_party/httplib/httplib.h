...
struct Response {
  ...
  void set_content_provider(const char* content_type,
                            ContentProviderWithoutLength provider,
                            ContentProviderResourceReleaser resource_releaser = nullptr);

  void set_chunked_content_provider(const char* content_type,
                                    ContentProviderWithoutLength provider,
                                    ContentProviderResourceReleaser resource_releaser = nullptr);
...
class ClientImpl {
 ...
 private:
  ...
  std::unique_ptr<Response> send_with_content_provider(Request& req,
                                                       ...
                                                       const char* content_type,
                                                       Error& error);
  Result send_with_content_provider(const char* method,
                                    const char* path,
                                    const Headers& headers,
                                    const char* body,
                                    size_t content_length,
...
socket_t create_client_socket(const char* host,
                              const char* ip,
                              int port,
                              int address_family,
                              bool tcp_nodelay,
...
inline bool process_server_socket(socket_t sock,
  ...
  return process_server_socket_core(
      sock, keep_alive_max_count, keep_alive_timeout_sec, [&](bool close_connection, bool& connection_closed) {
...
inline socket_t create_client_socket(const char* host,
  ...
  auto sock = create_socket(host, ip, port, address_family, 0, tcp_nodelay, std::move(socket_options),
                            [&](socket_t sock2, struct addrinfo& ai) -> bool {
                              ...
                              if (ret < 0) {
                                if (is_connection_error() || !wait_until_socket_is_ready(sock2, connection_timeout_sec,
...
inline bool can_compress_content_type(const std::string& content_type) {
  return (!content_type.find("text/") && content_type != "text/event-stream") || content_type == "image/svg+xml" ||
         content_type == "application/javascript" || content_type == "application/json" ||
         content_type == "application/xml" || content_type == "application/protobuf" ||
      ...
      if (decompressor->is_valid()) {
        ContentReceiverWithProgress out = [&](const char* buf, size_t n, uint64_t off, uint64_t len) {
          return decompressor->decompress(buf, n,
                                          [&](const char* buf2, size_t n2) { return receiver(buf2, n2, off, len); });
        };
...
class MultipartFormDataParser {
 ...
 private:
  ...
  bool start_with(const std::string& a, size_t off, const std::string& b) const {
...
inline void Response::set_content_provider(const char* content_type,
                                           ContentProviderWithoutLength provider,
                                           ContentProviderResourceReleaser resource_releaser) {
  set_header("Content-Type", content_type);
  content_length_ = 0;
...
inline void Response::set_chunked_content_provider(const char* content_type,
                                                   ContentProviderWithoutLength provider,
                                                   ContentProviderResourceReleaser resource_releaser) {
  set_header("Content-Type", content_type);
  content_length_ = 0;
...
inline socket_t Server::create_server_socket(const char* host,
                                             int port,
                                             int socket_flags,
...
inline bool Server::process_and_close_socket(socket_t sock) {
  auto ret = detail::process_server_socket(sock, keep_alive_max_count_, keep_alive_timeout_sec_, read_timeout_sec_,
...
inline socket_t ClientImpl::create_client_socket(Error& error) const {
  ...
  return detail::create_client_socket(host_.c_str(), ip.c_str(), port_, address_family_, tcp_nodelay_, socket_options_,
    ...
    if (content_provider) {
      ...
      data_sink.write = [&](const char* data, size_t data_len) -> bool {
        if (ok) {
          ...
          auto ret = compressor.compress(data, data_len, last, [&](const char* data, size_t data_len) {
...
inline Result ClientImpl::send_with_content_provider(const char* method,
                                                     const char* path,
                                                     const Headers& headers,
                                                     const char* body,
                                                     size_t content_length,
...
inline Result ClientImpl::Put(const char* path,
                              ...
                              ContentProvider content_provider,
                              const char* content_type) {
  return send_with_content_provider("PUT", path, headers, nullptr, content_length, std::move(content_provider), nullptr,
                                    content_type);
}
...
template <typename T>
inline bool process_server_socket_ssl(SSL* ssl,
                                      socket_t sock,
                                      size_t keep_alive_max_count,
                                      time_t keep_alive_timeout_sec,
                                      time_t read_timeout_sec,
...
inline bool SSLServer::process_and_close_socket(socket_t sock) {
  ...
  if (ssl) {
    ret = detail::process_server_socket_ssl(
        ssl, sock, keep_alive_max_count_, keep_alive_timeout_sec_, read_timeout_sec_, read_timeout_usec_,
