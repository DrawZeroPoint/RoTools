...
struct Expression_lhs {
  ...
  explicit Expression_lhs(L&& in, assertType::Enum at) : lhs(doctest::detail::forward<L>(in)), m_at(at) {}
  ...
  struct iter<std::tuple<Type, Rest...>> {                                                                       \
    iter(const char* file, unsigned line, int index) {                                                           \
      doctest::detail::regTest(                                                                                  \
          doctest::detail::TestCase(func<Type>, file, line, doctest_detail_test_suite_ns::getCurrentTestSuite(), \
...
// for logging
#define DOCTEST_INFO(...) \
  DOCTEST_INFO_IMPL(DOCTEST_ANONYMOUS(_DOCTEST_CAPTURE_), DOCTEST_ANONYMOUS(_DOCTEST_CAPTURE_), __VA_ARGS__)

#define DOCTEST_INFO_IMPL(mb_name, s_name, ...)                                                             \
  auto DOCTEST_ANONYMOUS(_DOCTEST_CAPTURE_) = doctest::detail::MakeContextScope([&](std::ostream* s_name) { \
...
Subcase::~Subcase() {
  if (m_entered) {
    ...
    if (std::uncaught_exception()
      ...
      DOCTEST_ITERATE_THROUGH_REPORTERS(test_case_exception, {"exception thrown in subcase - will translate later "
...
struct ConsoleReporter : public IReporter {
  ...
  void test_run_end(const TestRunStats& p) override {
    ...
    auto totwidth =
        int(std::ceil(log10((std::max(p.numTestCasesPassingFilters, static_cast<unsigned>(p.numAsserts))) + 1)));
    auto passwidth = int(std::ceil(log10((std::max(p.numTestCasesPassingFilters - p.numTestCasesFailed,
                                                   static_cast<unsigned>(p.numAsserts - p.numAssertsFailed))) +
                                         1)));
    auto failwidth =
        int(std::ceil(log10((std::max(p.numTestCasesFailed, static_cast<unsigned>(p.numAssertsFailed))) + 1)));
...
int Context::run() {
  ...
  for (auto& curr : testArray) {
    ...
    if ((p->last < p->numTestCasesPassingFilters && p->first <= p->last) || (p->first > p->numTestCasesPassingFilters))
      skip_me = true;
    ...
    // execute the test if it passes all the filtering
    {
      ...
      do {
        ...
        if (p->abort_after > 0 && p->numAssertsFailed + p->numAssertsFailedCurrentTest_atomic >= p->abort_after) {
          run_test = false;
  ...
  auto DOCTEST_FIX_FOR_MACOS_LIBCPP_IOSFWD_STRING_LINK_ERRORS = []() DOCTEST_NOINLINE { std::cout << std::string(); };
