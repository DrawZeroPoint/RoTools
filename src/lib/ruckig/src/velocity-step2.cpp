...
VelocityStep2::VelocityStep2(double tf,
                             double v0,
                             double a0,
                             double vf,
                             double af,
                             ...
                             double aMin,
                             double jMax)
    : v0(v0), a0(a0), tf(tf), vf(vf), af(af), _aMax(aMax), _aMin(aMin), _jMax(jMax) {}

bool VelocityStep2::time_acc0(Profile& profile, double aMax, double aMin, double jMax) {
  // UD Solution 1/2
  {
    const double h1 = Sqrt(
        (-a0 * a0 - af * af + 2 * a0 * af + 2 * jMax * ((a0 + af) * tf - 2 * (vf - v0))) / (jMax * jMax) + tf * tf);
  ...
  // UU Solution
  {
    ...
    profile.t[0] = -Power2(af - a0) / (2 * jMax * h1) + (vf - v0 - a0 * tf) / h1;
    profile.t[1] = h1 / jMax;
    profile.t[2] = 0;
    profile.t[3] = 0;
...
bool VelocityStep2::time_none(Profile& profile, double aMax, double aMin, double jMax) {
  ...
  // UD Solution 1/2
  {
    ...
    if (std::abs(jf) < std::abs(jMax) + 1e-12 &&
        profile.check_for_velocity_with_timing<JerkSigns::UDDU, Limits::NONE>(tf, jf, aMax, aMin)) {
      profile.pf = profile.p.back();
      return true;
    }
  }
