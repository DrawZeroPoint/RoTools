...
PositionStep1::PositionStep1(double p0,
                             double v0,
                             double a0,
                             double pf,
                             double vf,
                             ...
                             double aMin,
                             double jMax)
    : v0(v0), a0(a0), vf(vf), af(af), _vMax(vMax), _vMin(vMin), _aMax(aMax), _aMin(aMin), _jMax(jMax) {
  pd = pf - p0;
...
void PositionStep1::time_all_vel(Profile& profile,
                                 double vMax,
                                 double vMin,
                                 double aMax,
                                 double aMin,
  ...
  profile.t[3] =
      (3 * (a0_p4 * aMin - af_p4 * aMax) + 8 * aMax * aMin * (af_p3 - a0_p3 + 3 * jMax * (a0 * v0 - af * vf)) +
       6 * a0_a0 * aMin * (aMax * aMax - 2 * jMax * v0) - 6 * af_af * aMax * (aMin * aMin - 2 * jMax * vf) -
       12 * jMax *
  ...
  profile.t[1] = 0;
  profile.t[2] = t_acc0;
  profile.t[3] = -(3 * af_p4 - 8 * aMin * (af_p3 - a0_p3) - 24 * aMin * jMax * (a0 * v0 - af * vf) +
                   6 * af_af * (aMin * aMin - 2 * jMax * vf) -
                   12 * jMax *
                       (2 * aMin * jMax * pd + aMin * aMin * (vf + vMax) + jMax * (vMax * vMax - vf_vf) +
                        aMin * t_acc0 * (a0_a0 - 2 * jMax * (v0 + vMax)))) /
                 (24 * aMin * jMax_jMax * vMax);
  // profile.t[4] = -aMin/jMax;
  ...
  profile.t[1] = (a0_a0 / 2 - aMax * aMax - jMax * (v0 - vMax)) / (aMax * jMax);
  profile.t[2] = aMax / jMax;
  profile.t[3] = (3 * a0_p4 + 8 * aMax * (af_p3 - a0_p3) + 24 * aMax * jMax * (a0 * v0 - af * vf) +
                  6 * a0_a0 * (aMax * aMax - 2 * jMax * v0) -
                  12 * jMax *
                      (-2 * aMax * jMax * pd + aMax * aMax * (v0 + vMax) + jMax * (vMax * vMax - v0_v0) +
                       aMax * t_acc1 * (-af_af + 2 * (vf + vMax) * jMax))) /
                 (24 * aMax * jMax_jMax * vMax);
  profile.t[4] = t_acc1;
  profile.t[5] = 0;
  ...
  profile.t[1] = 0;
  profile.t[2] = t_acc0;
  profile.t[3] = (af_p3 - a0_p3) / (3 * jMax_jMax * vMax) +
...
void PositionStep1::time_acc0_acc1(Profile& profile,
                                   double vMax,
                                   double vMin,
                                   double aMax,
                                   double aMin,
  ...
  double h1 =
      (3 * (af_p4 * aMax - a0_p4 * aMin) +
       aMax * aMin * (8 * (a0_p3 - af_p3) + 3 * aMax * aMin * (aMax - aMin) + 6 * aMin * af_af - 6 * aMax * a0_a0) +
       12 * jMax *
           (aMax * aMin * ((aMax - 2 * a0) * v0 - (aMin - 2 * af) * vf) + aMin * a0_a0 * v0 - aMax * af_af * vf)) /
...
void PositionStep1::time_all_none_acc0_acc1(Profile& profile,
                                            double vMax,
                                            double vMin,
                                            double aMax,
                                            double aMin,
  ...
  const double h0_acc0 = 3 * (af_p4 - a0_p4) + 8 * (a0_p3 - af_p3) * aMax + 24 * aMax * jMax * (af * vf - a0 * v0) -
                         6 * a0_a0 * (aMax * aMax - 2 * jMax * v0) + 6 * af_af * (aMax * aMax - 2 * jMax * vf) +
                         12 * jMax * (jMax * (vf_vf - v0_v0 - 2 * aMax * pd) - aMax * aMax * (vf - v0));
  ...
  const double t_min_acc1 = -(a0 - aMin) / jMax;
  const double t_max_acc1 = (-a0 + aMax) / jMax;

  const double h0_acc1 = (a0_p4 - af_p4) / 4 + 2 * (af_p3 - a0_p3) * aMin / 3 + (a0_a0 - af_af) * aMin * aMin / 2 +
                         jMax * (af_af * vf + a0_a0 * v0 + 2 * aMin * (jMax * pd - a0 * v0 - af * vf) +
                                 aMin * aMin * (v0 + vf) + jMax * (v0_v0 - vf_vf));
  ...
  for (double t : roots_none) {
    ...
    if (t > DBL_EPSILON) {
      ...
      const double orig = -h2_h2 / (4 * jMax * t) + h2_none * (af / jMax + t) +
                          (4 * a0_p3 + 2 * af_p3 - 6 * a0_a0 * (af + 2 * jMax * t) + 12 * (af - a0) * jMax * v0 +
                           3 * jMax_jMax * (-4 * pd + (h1 + 8 * v0) * t)) /
                              (12 * jMax_jMax);
  ...
  for (double t : roots_acc1) {
    ...
    if (t > DBL_EPSILON) {
      ...
      double h1 = jMax * t;
      double orig = -(h0_acc1 / 2 + h1 * (h5 + a0 * (aMin - 2 * h1) * (aMin - h1) + a0_a0 * (5 * h1 / 2 - 2 * aMin) +
                                          aMin * aMin * h1 / 2 + jMax * (h1 / 2 - aMin) * (h1 * t + 2 * v0))) /
                    (aMin * jMax_jMax);
...
void PositionStep1::time_acc1_vel_two_step(Profile& profile,
                                           double vMax,
                                           double vMin,
                                           double aMax,
                                           double aMin,
  ...
  profile.t[3] = -(3 * af_p4 - 8 * aMin * (af_p3 - a0_p3) - 24 * aMin * jMax * (a0 * v0 - af * vf) +
                   ...
                   12 * jMax *
                       (2 * aMin * jMax * pd + aMin * aMin * (vf + vMax) + jMax * (vMax * vMax - vf_vf) +
                        aMin * a0 * (a0_a0 - 2 * jMax * (v0 + vMax)) / jMax)) /
                 (24 * aMin * jMax_jMax * vMax);
  profile.t[4] = -aMin / jMax;
  profile.t[5] = -(af_af / 2 - aMin * aMin + jMax * (vMax - vf)) / (aMin * jMax);
  profile.t[6] = profile.t[4] + af / jMax;
...
void PositionStep1::time_acc0_two_step(Profile& profile,
                                       double vMax,
                                       double vMin,
                                       double aMax,
                                       double aMin,
  ...
  // Three step - Removed aMax
  {
    ...
    const double h1 =
        std::sqrt(2 * (2 * h2 * h2 + h0 * (a0_p4 - 6 * a0_a0 * (af_af + 2 * jMax * vf) +
                                           8 * a0 * (af_p3 + 3 * jMax_jMax * pd + 3 * af * jMax * vf) -
                                           3 * (af_p4 + 4 * af_af * jMax * vf + 4 * jMax_jMax * (vf_vf - v0_v0))))) *
        Abs(jMax) / jMax;
    profile.t[0] = (4 * af_p3 + 2 * a0_p3 - 6 * a0 * af_af + 12 * jMax_jMax * pd + 12 * (af - a0) * jMax * vf + h1) /
                   (2 * jMax * h0);
    profile.t[1] = -h1 / (jMax * h0);
    profile.t[2] = (-4 * a0_p3 - 2 * af_p3 + 6 * a0_a0 * af + 12 * jMax_jMax * pd - 12 * (af - a0) * jMax * v0 + h1) /
                   (2 * jMax * h0);
    profile.t[3] = 0;
    profile.t[4] = 0;
    profile.t[5] = 0;
...
void PositionStep1::time_vel_two_step(Profile& profile,
                                      double vMax,
                                      double vMin,
                                      double aMax,
                                      double aMin,
  ...
  // Four step
  {
    ...
    profile.t[1] = 0;
    profile.t[2] = 0;
    profile.t[3] = (af_p3 - a0_p3) / (3 * jMax_jMax * vMax) + (a0 * v0 - af * vf + (af_af * h1) / 2) / (jMax * vMax) -
  ...
  // Four step
  {
    ...
    profile.t[3] = (af_p3 - a0_p3) / (3 * jMax_jMax * vMax) +
                   ...
                   (v0 / vMax + 1.0) * a0 / jMax - (vf / vMax + 1.0) * h1 + pd / vMax;
    profile.t[4] = h1;
    profile.t[5] = 0;
    profile.t[6] = h1 + af / jMax;
...
void PositionStep1::time_none_two_step(Profile& profile,
                                       double vMax,
                                       double vMin,
                                       double aMax,
                                       double aMin,
