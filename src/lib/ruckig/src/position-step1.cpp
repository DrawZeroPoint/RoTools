...
PositionStep1::PositionStep1(double p0,
                             ...
                             double aMin,
                             double jMax)
    : p0(p0), v0(v0), a0(a0), pf(pf), vf(vf), af(af), _vMax(vMax), _vMin(vMin), _aMax(aMax), _aMin(aMin), _jMax(jMax) {
  pd = pf - p0;
...
void PositionStep1::time_all_vel(Profile& profile, double vMax, double vMin, double aMax, double aMin, double jMax) {
  ...
  profile.t[1] = (a0_a0 / 2 - aMax * aMax - jMax * (v0 - vMax)) / (aMax * jMax);
  profile.t[2] = aMax / jMax;
  profile.t[3] = (3 * a0_p4 + 8 * (af_p3 - a0_p3) * aMax + 24 * aMax * jMax * (a0 * v0 - af * vf) +
                  6 * a0_a0 * (aMax * aMax - 2 * jMax * v0) -
                  12 * jMax *
                      (-2 * aMax * jMax * pd + aMax * aMax * (v0 + vMax) + jMax * (vMax * vMax - v0_v0) +
                       (2 * (vf + vMax) * jMax - af_af) * aMax * t_acc1)) /
...
void PositionStep1::time_acc1(Profile& profile, double vMax, double vMin, double aMax, double aMin, double jMax) {
  ...
  const double h0 = (a0_p4 - af_p4) / 4 + 2 * (af_p3 - a0_p3) * aMin / 3 + (a0_a0 - af_af) * aMin * aMin / 2 +
  ...
  for (double t : roots) {
    ...
    if (t > DBL_EPSILON) {
      ...
      double h1 = jMax * t;
      double orig = -(h0 / 2 + h1 * (h5 + a0 * (aMin - 2 * h1) * (aMin - h1) + a0_a0 * (5 * h1 / 2 - 2 * aMin) +
      ...
      h1 = jMax * t;
      orig = -(h0 / 2 + h1 * (h5 + a0 * (aMin - 2 * h1) * (aMin - h1) + a0_a0 * (5 * h1 / 2 - 2 * aMin) +
                              aMin * aMin * h1 / 2 + jMax * (h1 / 2 - aMin) * (h1 * t + 2 * v0))) /
             (aMin * jMax_jMax);
...
void PositionStep1::time_acc0(Profile& profile, double vMax, double vMin, double aMax, double aMin, double jMax) {
  ...
  const double h0 = 3 * (af_p4 - a0_p4) + 8 * (a0_p3 - af_p3) * aMax + 24 * aMax * jMax * (af * vf - a0 * v0) -
  ...
  // UDDU / UDUD modern, this one is in particular prone to numerical issues
  {
    ...
    for (double t : roots) {
      ...
      if (t > DBL_EPSILON) {
        ...
        const double orig = -h2_h2 / (4 * jMax * t) + h2 * (af / jMax + t) +
