...
PYBIND11_MODULE(ruckig, m) {
  m.doc() =
      "Instantaneous Motion Generation for Robots and Machines. Real-time and time-optimal trajectory calculation \
...
limited by velocity, acceleration, and jerk constraints.";
  ...
  py::class_<Trajectory<DynamicDOFs>>(m, "Trajectory")
      ...
      .def(
          "at_time",
          [](const Trajectory<DynamicDOFs>& traj, double time, bool return_section = false) {
            std::vector<double> new_position(traj.degrees_of_freedom), new_velocity(traj.degrees_of_freedom),
                new_acceleration(traj.degrees_of_freedom);
            size_t new_section;
      ...
      .def(
          ...
          [](const Trajectory<DynamicDOFs>& traj, size_t dof, double position) -> py::object {
            double time;
  ...
  py::class_<OutputParameter<DynamicDOFs>>(m, "OutputParameter")
      ...
      .def("__copy__", [](const OutputParameter<DynamicDOFs>& self) { return OutputParameter<DynamicDOFs>(self); });
  ...
  py::class_<Ruckig<DynamicDOFs, true>>(m, "Ruckig")
      ...
      .def("filter_intermediate_positions", &Ruckig<DynamicDOFs, true>::filter_intermediate_positions, "input"_a,
      ...
      .def("validate_input", &Ruckig<DynamicDOFs, true>::validate_input, "input"_a,
           "check_current_state_within_limits"_a = false, "check_target_state_within_limits"_a = true)
      .def("calculate",
           static_cast<Result (Ruckig<DynamicDOFs, true>::*)(
               const InputParameter<DynamicDOFs>&, Trajectory<DynamicDOFs>&)>(&Ruckig<DynamicDOFs, true>::calculate),
           "input"_a, "trajectory"_a)
      .def("calculate",
           static_cast<Result (Ruckig<DynamicDOFs, true>::*)(const InputParameter<DynamicDOFs>&,
                                                             Trajectory<DynamicDOFs>&, bool&)>(
               &Ruckig<DynamicDOFs, true>::calculate),
           "input"_a, "trajectory"_a, "was_interrupted"_a)
