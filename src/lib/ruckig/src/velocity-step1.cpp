...
VelocityStep1::VelocityStep1(double v0, double a0, double vf, double af, double aMax, double aMin, double jMax)
    : v0(v0), a0(a0), vf(vf), af(af), _aMax(aMax), _aMin(aMin), _jMax(jMax) {}
...
bool VelocityStep1::get_profile(const Profile& input, Block& block) {
  ...
  if (std::abs(v0) < DBL_EPSILON && std::abs(vf) < DBL_EPSILON && std::abs(a0) < DBL_EPSILON &&
