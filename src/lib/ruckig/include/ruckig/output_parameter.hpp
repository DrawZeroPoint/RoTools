...
class OutputParameter {
 ...
 public:
  ...
  OutputParameter(size_t max_number_of_waypoints)
      : degrees_of_freedom(DOFs), trajectory(Trajectory<DOFs>(max_number_of_waypoints)) {}

  template <size_t D = DOFs, typename std::enable_if<D == 0, int>::type = 0>
  OutputParameter(size_t dofs, size_t max_number_of_waypoints)
      : degrees_of_freedom(dofs), trajectory(Trajectory<0>(dofs, max_number_of_waypoints)) {
    resize(dofs);
  }
#endif
