...
class Profile {
 public:
  ...
  bool check_for_velocity(double jf, double aMax, double aMin) {
    ...
    // std::cout << std::setprecision(15) << "target: " << std::abs(p.back() - pf) << " " << std::abs(v.back() - vf) <<
    // " " << std::abs(a.back() - af) << " T: " << t_sum.back() << " " << to_string() << std::endl;
    return std::abs(v.back() - vf) < 1e-8 && std::abs(a.back() - af) < 1e-10 && a[1] >= aLowLim && a[3] >= aLowLim &&
           a[5] >= aLowLim && a[1] <= aUppLim && a[3] <= aUppLim && a[5] <= aUppLim;
  }
  ...
  bool check(double jf, double vMax, double vMin, double aMax, double aMin) {
    ...
    if constexpr (limits == Limits::ACC0_ACC1_VEL || limits == Limits::ACC0_VEL || limits == Limits::ACC1_VEL ||
                  limits == Limits::VEL) {
    ...
    for (size_t i = 0; i < 7; ++i) {
      ...
      if constexpr (limits == Limits::ACC0_ACC1_VEL || limits == Limits::ACC0_ACC1 || limits == Limits::ACC0_VEL ||
                    limits == Limits::ACC1_VEL || limits == Limits::VEL) {
        if (i == 2) {
          a[3] = 0.0;
        }
      }
    ...
    // Velocity limit can be broken in the beginning if both initial velocity and acceleration are too high
    // std::cout << std::setprecision(16) << "target: " << std::abs(p.back() - pf) << " " << std::abs(v.back() - vf) <<
    // " " << std::abs(a.back() - af) << " T: " << t_sum.back() << " " << to_string() << std::endl;
    return std::abs(p.back() - pf) < 1e-8 && std::abs(v.back() - vf) < 1e-8 && std::abs(a.back() - af) < 1e-10 &&
           a[1] >= aLowLim && a[3] >= aLowLim && a[5] >= aLowLim && a[1] <= aUppLim && a[3] <= aUppLim &&
           a[5] <= aUppLim && v[3] <= vUppLim && v[4] <= vUppLim && v[5] <= vUppLim && v[6] <= vUppLim &&
  ...
  inline bool check_with_timing(double tf, double jf, double vMax, double vMin, double aMax, double aMin, double jMax) {
    return (std::abs(jf) < std::abs(jMax) + 1e-12) &&
           check_with_timing<jerk_signs, limits>(tf, jf, vMax, vMin, aMax, aMin);
  }
  ...
  inline static void check_position_extremum(double t_ext,
                                             double t_sum,
                                             double t,
                                             double p,
                                             double v,
  ...
  static void check_step_for_position_extremum(double t_sum,
                                               double t,
                                               double p,
                                               double v,
                                               double a,
  ...
  PositionExtrema get_position_extrema() const {
    ...
    if (brake.duration > 0.0) {
      if (brake.t[0] > 0.0) {
        ...
        if (brake.t[1] > 0.0) {
          check_step_for_position_extremum(brake.t[0], brake.t[1], brake.p[1], brake.v[1], brake.a[1], brake.j[1],
                                           extrema);
        }
      }
    }
  ...
  std::string to_string() const {
    ...
    switch (limits) {
      ...
      case Limits::ACC1:
        result += "ACC1";
        break;
      case Limits::ACC0_ACC1:
        result += "ACC0_ACC1";
        break;
      case Limits::ACC0_VEL:
        result += "ACC0_VEL";
        break;
      case Limits::ACC1_VEL:
        result += "ACC1_VEL";
        break;
      case Limits::NONE:
        result += "NONE";
