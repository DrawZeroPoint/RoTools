...
class Trajectory {
 ...
 public:
  ...
  //! The Python wrapper takes `time` as an argument, and returns `new_position`, `new_velocity`, and `new_acceleration`
  //! instead.
  void at_time(double time,
               Vector<double>& new_position,
               Vector<double>& new_velocity,
               Vector<double>& new_acceleration,
               size_t& new_section) const {
    if constexpr (DOFs == 0) {
      if (degrees_of_freedom != new_position.size() || degrees_of_freedom != new_velocity.size() ||
          degrees_of_freedom != new_acceleration.size()) {
    ...
    if (time >= duration) {
      ...
      for (size_t dof = 0; dof < degrees_of_freedom; ++dof) {
        const double t_pre =
            (profiles.size() > 1) ? cumulative_times[cumulative_times.size() - 2] : profiles_dof[dof].brake.duration;
        const double t_diff = time - (t_pre + profiles_dof[dof].t_sum.back());
        std::tie(new_position[dof], new_velocity[dof], new_acceleration[dof]) =
            integrate(t_diff, profiles_dof[dof].p.back(), profiles_dof[dof].v.back(), profiles_dof[dof].a.back(), 0);
      }
      return;
    }
    ...
    for (size_t dof = 0; dof < degrees_of_freedom; ++dof) {
      ...
      if (new_section == 0 && p.brake.duration > 0) {
        if (t_diff_dof < p.brake.duration) {
          ...
          std::tie(new_position[dof], new_velocity[dof], new_acceleration[dof]) =
              integrate(t_diff_dof, p.brake.p[index], p.brake.v[index], p.brake.a[index], p.brake.j[index]);
          continue;
        } else {
      ...
      //             std::tie(new_position[dof], new_velocity[dof], new_acceleration[dof]) = integrate(t_diff_dof,
      //             p.p.back(), p.v.back(), p.a.back(), p.accel.j[1]); continue;
      //         }
      ...
      //         std::tie(new_position[dof], new_velocity[dof], new_acceleration[dof]) = integrate(t_diff_dof,
      //         p.accel.p[index], p.accel.v[index], p.accel.a[index], p.accel.j[index]); continue;
      //     }
      // }
      ...
      if (t_diff_dof >= p.t_sum.back()) {
        // Keep constant acceleration
        std::tie(new_position[dof], new_velocity[dof], new_acceleration[dof]) =
            integrate(t_diff_dof - p.t_sum.back(), p.p.back(), p.v.back(), p.a.back(), 0);
        continue;
      }
      ...
      std::tie(new_position[dof], new_velocity[dof], new_acceleration[dof]) =
          integrate(t_diff_dof, p.p[index_dof], p.v[index_dof], p.a[index_dof], p.j[index_dof]);
    }
  }
  ...
  void at_time(double time,
               Vector<double>& new_position,
               Vector<double>& new_velocity,
               Vector<double>& new_acceleration) const {
    size_t new_section;
