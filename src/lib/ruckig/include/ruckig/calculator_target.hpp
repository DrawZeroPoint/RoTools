...
class TargetCalculator {
 private:
  ...
  template <class T>
  using VectorIntervals = typename std::conditional<DOFs >= 1, std::array<T, 3 * DOFs + 1>, std::vector<T>>::type;
  ...
  bool is_input_collinear(const InputParameter<DOFs>& inp,
    ...
    const double max_jerk_limiting =
        (limiting_direction == Profile::Direction::UP) ? jMax[limiting_dof] : -jMax[limiting_dof];
    ...
    for (size_t dof = 0; dof < pd.size(); ++dof) {
      ...
      if (std::abs(inp.current_velocity[dof] - v0_scale * pd[dof]) > eps_colinear ||
  ...
  bool synchronize(std::optional<double> t_min,
                   double& t_sync,
                   int& limiting_dof,
    ...
    for (size_t dof = 0; dof < degrees_of_freedom; ++dof) {
      ...
      possible_t_syncs[degrees_of_freedom + dof] =
          blocks[dof].a ? blocks[dof].a->right : std::numeric_limits<double>::infinity();
      possible_t_syncs[2 * degrees_of_freedom + dof] =
          blocks[dof].b ? blocks[dof].b->right : std::numeric_limits<double>::infinity();
    ...
    std::sort(idx.begin(), idx_end, [&possible_t_syncs = possible_t_syncs](size_t i, size_t j) {
      return possible_t_syncs[i] < possible_t_syncs[j];
    });
 ...
 public:
  ...
  Result calculate(const InputParameter<DOFs>& inp, Trajectory<DOFs>& traj, double delta_time, bool& was_interrupted) {
    ...
    for (size_t dof = 0; dof < degrees_of_freedom; ++dof) {
      ...
      inp_per_dof_control_interface[dof] =
          inp.per_dof_control_interface ? inp.per_dof_control_interface.value()[dof] : inp.control_interface;
      inp_per_dof_synchronization[dof] =
          inp.per_dof_synchronization ? inp.per_dof_synchronization.value()[dof] : inp.synchronization;
      ...
      switch (inp_per_dof_control_interface[dof]) {
        case ControlInterface::Position: {
          p.brake.get_position_brake_trajectory(inp.current_velocity[dof], inp.current_acceleration[dof],
                                                inp.max_velocity[dof], inp_min_velocity[dof], inp.max_acceleration[dof],
                                                inp_min_acceleration[dof], inp.max_jerk[dof]);
          // p.accel.get_position_brake_trajectory(inp.target_velocity[dof], inp.target_acceleration[dof],
          // inp.max_velocity[dof], inp_min_velocity[dof], inp.max_acceleration[dof], inp_min_acceleration[dof],
          // inp.max_jerk[dof]);
          p.set_boundary(inp.current_position[dof], inp.current_velocity[dof], inp.current_acceleration[dof],
                         inp.target_position[dof], inp.target_velocity[dof], inp.target_acceleration[dof]);
        } break;
        case ControlInterface::Velocity: {
          p.brake.get_velocity_brake_trajectory(inp.current_acceleration[dof], inp.max_acceleration[dof],
                                                inp_min_acceleration[dof], inp.max_jerk[dof]);
          // p.accel.get_velocity_brake_trajectory(inp.target_acceleration[dof], inp.max_acceleration[dof],
          // inp_min_acceleration[dof], inp.max_jerk[dof]);
          p.set_boundary(inp.current_position[dof], inp.current_velocity[dof], inp.current_acceleration[dof],
      ...
      switch (inp_per_dof_control_interface[dof]) {
        case ControlInterface::Position: {
          PositionStep1 step1{p.p[0],
                              ...
                              inp_min_velocity[dof],
                              inp.max_acceleration[dof],
                              inp_min_acceleration[dof],
                              inp.max_jerk[dof]};
          found_profile = step1.get_profile(p, blocks[dof]);
        } break;
        case ControlInterface::Velocity: {
          VelocityStep1 step1{
              p.v[0], p.a[0], p.vf, p.af, inp.max_acceleration[dof], inp_min_acceleration[dof], inp.max_jerk[dof]};
          found_profile = step1.get_profile(p, blocks[dof]);
        } break;
      }
      ...
      if (!found_profile) {
        if constexpr (throw_error) {
          throw std::runtime_error("[ruckig] error in step 1, dof: " + std::to_string(dof) +
                                   " input: " + inp.to_string());
        }
    ...
    const bool found_synchronization =
        synchronize(inp.minimum_duration, traj.duration, limiting_dof, traj.profiles[0], discrete_duration, delta_time);
    ...
    if (std::any_of(inp_per_dof_synchronization.begin(), inp_per_dof_synchronization.end(),
      ...
      if (is_input_collinear(inp, inp.max_jerk, p_limiting.direction, limiting_dof, new_max_jerk)) {
        ...
        for (size_t dof = 0; dof < degrees_of_freedom; ++dof) {
          ...
          switch (p.jerk_signs) {
            case Profile::JerkSigns::UDDU: {
              if (!p.check_with_timing<Profile::JerkSigns::UDDU, Profile::Limits::NONE>(
                      t_profile, new_max_jerk[dof], inp.max_velocity[dof], inp_min_velocity[dof],
                      inp.max_acceleration[dof], inp_min_acceleration[dof], inp.max_jerk[dof])) {
                found_time_synchronization = false;
              }
            } break;
            case Profile::JerkSigns::UDUD: {
              if (!p.check_with_timing<Profile::JerkSigns::UDUD, Profile::Limits::NONE>(
                      t_profile, new_max_jerk[dof], inp.max_velocity[dof], inp_min_velocity[dof],
                      inp.max_acceleration[dof], inp_min_acceleration[dof], inp.max_jerk[dof])) {
                found_time_synchronization = false;
              }
        ...
        if (found_time_synchronization &&
            std::all_of(inp_per_dof_synchronization.begin(), inp_per_dof_synchronization.end(),
                        [](Synchronization s) { return s == Synchronization::Phase || s == Synchronization::None; })) {
          return Result::Working;
        }
      }
    }
    ...
    for (size_t dof = 0; dof < degrees_of_freedom; ++dof) {
      ...
      if (inp_per_dof_synchronization[dof] == Synchronization::TimeIfNecessary &&
          std::abs(inp.target_velocity[dof]) < eps && std::abs(inp.target_acceleration[dof]) < eps) {
        p = blocks[dof].p_min;
        continue;
      }
      ...
      if (!found_time_synchronization) {
        if constexpr (throw_error) {
          throw std::runtime_error("[ruckig] error in step 2 in dof: " + std::to_string(dof) +
                                   " for t sync: " + std::to_string(traj.duration) + " input: " + inp.to_string());
        }
        return Result::ErrorSynchronizationCalculation;
      }
