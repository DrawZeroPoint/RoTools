...
class Ruckig {
  ...
  inline static double v_at_a_zero(double v0, double a0, double j) { return v0 + (a0 * a0) / (2 * j); }
 ...
 public:
  ...
  explicit Ruckig(size_t dofs)
      : degrees_of_freedom(dofs),
        delta_time(-1.0),
        calculator(Calculator<0>(dofs)),
        max_number_of_waypoints(0),
        current_input(InputParameter<0>(dofs)) {}
  ...
  std::vector<Vector<double>> filter_intermediate_positions(const InputParameter<DOFs>& input,
    ...
    for (size_t dof = 0; dof < degrees_of_freedom; ++dof) {
      ...
      if (control_interface == ControlInterface::Position) {
        ...
        if (check_current_state_within_limits &&
            ((a0 > 0 && v_at_a_zero(v0, a0, jMax) > vMax) || (a0 < 0 && v_at_a_zero(v0, a0, -jMax) < vMin))) {
          return false;
        }
        if (check_target_state_within_limits &&
            ((af < 0 && v_at_a_zero(vf, af, jMax) > vMax) || (af > 0 && v_at_a_zero(vf, af, -jMax) < vMin))) {
          return false;
        }
      }
    }
  ...
  Result calculate(const InputParameter<DOFs>& input, Trajectory<DOFs>& trajectory, bool& was_interrupted) {
    ...
    return calculator.template calculate<throw_error, return_error_at_maximal_duration>(input, trajectory, delta_time,
                                                                                        was_interrupted);
  }
