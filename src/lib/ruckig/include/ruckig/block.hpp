...
class Block {
  struct Interval {
    ...
    explicit Interval(const Profile& profile_left, const Profile& profile_right) {
      const double left_duration =
          profile_left.t_sum.back() + profile_left.brake.duration + profile_left.accel.duration;
      const double right_duration =
          profile_right.t_sum.back() + profile_right.brake.duration + profile_right.accel.duration;
  ...
  template <size_t N>
  inline static void remove_profile(std::array<Profile, N>& valid_profiles,
 ...
 public:
  ...
  static bool calculate_block(Block& block, std::array<Profile, N>& valid_profiles, size_t valid_profile_counter) {
    ...
    if (valid_profile_counter == 1) {
      ...
      if (std::abs(valid_profiles[0].t_sum.back() - valid_profiles[1].t_sum.back()) <
      ...
      } else if (std::abs(valid_profiles[2].t_sum.back() - valid_profiles[3].t_sum.back()) <
      ...
      } else if (std::abs(valid_profiles[0].t_sum.back() - valid_profiles[3].t_sum.back()) <
                     256 * std::numeric_limits<double>::epsilon() &&
    ...
    const auto idx_min_it =
        std::min_element(valid_profiles.cbegin(), valid_profiles.cbegin() + valid_profile_counter,
                         [](const Profile& a, const Profile& b) { return a.t_sum.back() < b.t_sum.back(); });
