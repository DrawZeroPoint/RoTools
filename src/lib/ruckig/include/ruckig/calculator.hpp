...
class Calculator {
 ...
 public:
  ...
  explicit Calculator(size_t dofs)
      : target_calculator(TargetCalculator<DOFs>(dofs)), waypoints_calculator(WaypointsCalculator<DOFs>(dofs)) {}

  template <size_t D = DOFs, typename std::enable_if<D == 0, int>::type = 0>
  explicit Calculator(size_t dofs, size_t max_waypoints)
      : target_calculator(TargetCalculator<DOFs>(dofs)),
        waypoints_calculator(WaypointsCalculator<DOFs>(dofs, max_waypoints)) {}
#else
  template <size_t D = DOFs, typename std::enable_if<D == 0, int>::type = 0>
  ...
  Result calculate(const InputParameter<DOFs>& input,
    ...
    if (use_waypoints_trajectory(input)) {
      result = waypoints_calculator.template calculate<throw_error, return_error_at_maximal_duration>(
          input, trajectory, delta_time, was_interrupted);
    } else {
      result = target_calculator.template calculate<throw_error, return_error_at_maximal_duration>(
          input, trajectory, delta_time, was_interrupted);
    }
#else
    result = target_calculator.template calculate<throw_error, return_error_at_maximal_duration>(
  ...
  template <bool throw_error, bool return_error_at_maximal_duration>
  Result continue_calculation(const InputParameter<DOFs>& input,
                              Trajectory<DOFs>& trajectory,
                              double delta_time,
                              bool& was_interrupted) {
    Result result;
#if defined WITH_ONLINE_CLIENT
    if (use_waypoints_trajectory(input)) {
      result = waypoints_calculator.template continue_calculation<throw_error, return_error_at_maximal_duration>(
          input, trajectory, delta_time, was_interrupted);
    } else {
      result = target_calculator.template continue_calculation<throw_error, return_error_at_maximal_duration>(
          input, trajectory, delta_time, was_interrupted);
    }
#else
    result = target_calculator.template continue_calculation<throw_error, return_error_at_maximal_duration>(
