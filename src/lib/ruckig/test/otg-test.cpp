...
size_t random_1, random_3, random_3_high, step_through_3, random_discrete_3, random_direction_3, comparison_1,
    comparison_3, velocity_random_3;
...
inline void check_calculation(OTGType& otg, InputParameter<DOFs>& input) {
  ...
  for (size_t dof = 0; dof < otg.degrees_of_freedom; ++dof) {
    CHECK_FALSE((std::isnan(output.new_position[dof]) || std::isnan(output.new_velocity[dof]) ||
                 std::isnan(output.new_acceleration[dof])));
  }
}
...
TEST_CASE("per-dof-setting" * doctest::description("Per DoF Settings")) {
  ...
  input.per_dof_control_interface = {ControlInterface::Position, ControlInterface::Velocity,
...
TEST_CASE("random_discrete_3" *
          doctest::description("Random discrete input with 3 DoF and target velocity, acceleration")) {
...
TEST_CASE("velocity_random_3" *
          doctest::description("Random input with 3 DoF and target velocity, acceleration in velocity control")) {
  constexpr size_t DOFs{3};
  Ruckig<DOFs, true> otg{0.005};
  InputParameter<DOFs> input;
  input.control_interface = ControlInterface::Velocity;
...
TEST_CASE("random_3_high" *
          doctest::description("Random input with 3 DoF and target velocity, acceleration and high limits")) {
  constexpr size_t DOFs{3};
  Ruckig<DOFs, true> otg{0.005};
  InputParameter<DOFs> input;
...
TEST_CASE("step_through_3" *
          doctest::description("Step through random input with 3 DoF and target velocity, acceleration")) {
  constexpr size_t DOFs{3};
...
TEST_CASE(
    "random_direction_3" *
    doctest::description("Random input with 3 DoF and target velocity, acceleration and min velocity, acceleration")) {
  constexpr size_t DOFs{3};
  Ruckig<DOFs, true> otg{0.005};
  InputParameter<DOFs> input;
...
int main(int argc, char** argv) {
  ...
  const size_t remainder =
      number_trajectories - (random_1 + step_through_3 + random_direction_3 + comparison_1 + comparison_3 +
                             velocity_random_3 + random_discrete_3);  // 1. Normal, 2. High
  random_3 = (size_t)(remainder * 95 / 100);
  random_3_high = (size_t)(remainder * 5 / 100);
  std::cout << "<number_trajectories> Random (1 DoF): " << random_1 << " (3 DoF): " << random_3
            << " High Limits (3 DoF): " << random_3_high << "  Step Through (3 Dof): " << step_through_3
            << "  Comparison (1 DoF): " << comparison_1 << " (3 DoF): " << comparison_3
            << " Total: " << number_trajectories << std::endl;
