...
RuckigOptimizer::RuckigOptimizer(int dof,
                                 ...
                                 double reduce_ratio) {
  if (dof > 28) {
    ROS_ERROR("RoTools: DOF %d exceed the capacity (28) of the online trajectory optimizer", dof);
    return;
  }
  dof_ = new int(dof);
  initialized_ = new bool(false);

  trajectory_generator_ = new ruckig::Ruckig<28>(1. / frequency);
  input_param_ = new ruckig::InputParameter<28>();
  output_param_ = new ruckig::OutputParameter<28>();
...
void RuckigOptimizer::init(const sensor_msgs::JointState& msg, const std::vector<double>& q_d) {
  std::array<double, 28> position{};
  std::array<double, 28> velocity{};
  std::array<double, 28> acceleration{};
...
void RuckigOptimizer::update(std::vector<double>& q_cmd, std::vector<double>& dq_cmd) {
  ...
  //  FIXME add effort
  std::array<double, 28> new_position = output_param_->new_position;
  std::array<double, 28> new_velocity = output_param_->new_velocity;
