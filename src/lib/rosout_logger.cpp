...
RosoutLogger::RosoutLogger(TreeNode* root_node, ros::console::Level verbosity_level)
    : StatusChangeLogger(root_node), _level(verbosity_level) {
  bool expected = false;
...
void RosoutLogger::callback(Duration timestamp, const TreeNode& node, NodeStatus prev_status, NodeStatus status) {
  ...
  switch (_level) {
    case ros::console::Level::Debug:
      ROS_DEBUG("[%s%s]: %s -> %s", node_name.c_str(), &whitespaces[std::min(ws_count, node_name.size())],
                toStr(prev_status, true).c_str(), toStr(status, true).c_str());
      break;

    case ros::console::Level::Info:
      ROS_INFO("[%s%s]: %s -> %s", node_name.c_str(), &whitespaces[std::min(ws_count, node_name.size())],
               toStr(prev_status, true).c_str(), toStr(status, true).c_str());
      break;
    default:
      ROS_INFO("[%s%s]: %s -> %s", node_name.c_str(), &whitespaces[std::min(ws_count, node_name.size())],
