...
class PlannerServer(EStop):
    ...
    def __init__(self, kwargs):
        ...
        rate = kwargs['rate']
        self.plan_execution_timer = rospy.Timer(rospy.Duration(1.0 / rate), self.execute_plan_handle)
            ...
            if self._mode == 0:
                ...
                while self.enable:
                    ...
                    if cnt >= len(trajectories):
                        # TODO avoid state jump
                        cnt = 0
        else:
            rospy.logwarn_throttle(3, 'Cannot get plan')

    def _publish_poses(self, poses):
        """The poses order should be semantically the same as the publishers.

        """
        assert len(poses) == len(self._control_publishers), \
            rospy.logerr('Target poses and publisher number mismatch')
