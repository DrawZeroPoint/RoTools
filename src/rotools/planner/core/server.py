...
class PlannerServer(EStop):
    ...
    def __init__(self, kwargs):
        ...
        mode = kwargs["mode"]
        assert mode in [0, 1, 2], rospy.logerr(
            "Mode should be 0 (step once), 1 (loop once), or 2 (infinite loop),"
        ...
        self.srv_pub_switch = rospy.Service(
            "/planner/enable", SetBool, self.pub_switch_handle
        )

        self.srv_initialize = rospy.Service(
            "/planner/initialize", Trigger, self.initialize_handle
        )
        ...
        assert isinstance(control_topics, list), rospy.logerr(
            "control_topics is not a list"
        )
        ...
        self.plan_execution_timer = rospy.Timer(
            rospy.Duration.from_sec(0.01), self.execute_plan_handle
        )
    ...
    def execute_plan_handle(self, event):
        ...
        if not self._pose_initialized:
            rospy.logwarn_throttle(
                3, "Poses not initialized, call the service /planner/initialize first"
            )
            return
        ...
        if ok:
            if self._mode == 0:
                ...
                for target_poses in trajectories:
                    if self.enabled:
                        self._publish_poses(target_poses)
                        rospy.sleep(rospy.Duration.from_sec(1.0 / self.rate))
                self.set_status(False)
                self._pose_initialized = False
            else:
                cnt = 0
    ...
    def _publish_poses(self, poses):
        ...
        assert len(poses) == self._n_topics, rospy.logerr(
            "Target poses {} and publisher number {} mismatch".format(
                len(poses), self._n_topics
            )
        )
        for i, pose in enumerate(poses):
            assert isinstance(pose, PoseStamped), rospy.logerr(
                "Target pose type is not PoseStamped"
            )
