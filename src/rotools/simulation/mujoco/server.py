...
class MuJoCoServer(object):
    ...
    def __init__(self, kwargs):
        ...
        # Provided services
        self._execute_gripper_control_srv = rospy.Service(
            "execute_gripper_control",
            ExecuteBinaryAction,
            self.execute_gripper_control_handle,
        )

        self._execute_object_reset_srv = rospy.Service(
            "execute_object_reset",
            ExecuteBinaryAction,
            self.execute_object_reset_handle,
        )

        self._execute_verbose_reset_srv = rospy.Service(
            "execute_verbose_reset",
            ExecuteBinaryAction,
            self.execute_verbose_reset_handle,
        )

        self._get_pose_srv = rospy.Service(
        ...
        if "joint_command_topic_id" in kwargs.keys():
            ...
            self.joint_command_subscriber = rospy.Subscriber(
                joint_command_topic_id, JointState, self.joint_command_cb
            )

        if "base_command_topic_id" in kwargs.keys():
            base_command_topic_id = kwargs["base_command_topic_id"]
            self.base_command_subscriber = rospy.Subscriber(
                base_command_topic_id, Twist, self.base_command_cb
            )
        ...
        if "joint_state_topic_id" in kwargs.keys():
            ...
            self.joint_state_publisher = rospy.Publisher(
                joint_state_topic_id, JointState, queue_size=1
            )
        ...
        if "object_pose_topic_id" in kwargs.keys():
            ...
            self.object_pose_publisher = rospy.Publisher(
                object_pose_topic_id, Pose, queue_size=1
            )
        else:
            self.object_pose_publisher = None

        self._timer = rospy.Timer(
            rospy.Duration.from_sec(1.0 / rate), self.publish_handle
        )
        if kwargs["verbose"]:
            self._verbose_reset_timer = rospy.Timer(
                rospy.Duration.from_sec(5), self.execute_verbose_reset_handle
            )

        # Get robot base params if available
        self.wheel_radius = (
            kwargs["wheel_radius"] if "wheel_radius" in kwargs.keys() else None
        )
        self.base_width = (
            kwargs["base_width"] if "base_width" in kwargs.keys() else None
        )
        self.base_length = (
    ...
    def joint_command_cb(self, cmd):
        if len(cmd.position) != len(cmd.velocity) or len(cmd.position) != len(
            cmd.effort
        ):
            rospy.logwarn_throttle(
                3,
                "Joint command should contain position, velocity, and effort. "
    ...
    def base_command_cb(self, cmd):
        if (
            self.wheel_radius is None
            or self.base_width is None
            or self.base_length is None
        ):
            rospy.logwarn_throttle(
                1,
                "Base param not set: wheel_radius {}, base_width {}, base_length {}".format(
                    self.wheel_radius, self.base_width, self.base_length
                ),
            )
            return
        vel = mecanum_base_get_wheel_velocities(
            cmd, self.wheel_radius, self.base_width, self.base_length
        )
