...
class MoveItServer(object):
    ...
    def __init__(self, kwargs):
        ...
        self._srv_get_names = rospy.Service(
            "get_all_group_names", GetAllNames, self.get_all_names_handle
        )
        self._srv_get_act_names = rospy.Service(
            "get_active_group_names", GetAllNames, self.get_active_names_handle
        )
        self._srv_get_pose = rospy.Service(
            "get_group_pose", GetGroupPose, self.get_pose_handle
        )
        self._srv_get_js = rospy.Service(
            "get_group_joint_states", GetGroupJointStates, self.get_js_handle
        )
        ...
        self._srv_group_shift = rospy.Service(
            "execute_group_shift", ExecuteGroupShift, self.group_shift_handle
        )
        self._srv_group_pose = rospy.Service(
            "execute_group_pose", ExecuteGroupPose, self.group_pose_handle
        )
        self._srv_group_js = rospy.Service(
            "execute_group_joint_states", ExecuteGroupJointStates, self.group_js_handle
        )
        self._srv_group_home = rospy.Service(
            "execute_group_named_states",
            ExecuteGroupNamedStates,
            self.group_named_states_handle,
        )
        self._srv_group_plan = rospy.Service(
            "execute_group_plan", ExecuteGroupPlan, self.group_plan_handle
        )

        # For multiple groups
        self._srv_all_pose = rospy.Service(
            "execute_all_poses_py", ExecuteAllPoses, self.all_poses_handle
        )

        # Planning scene helpers
        self._srv_add_box = rospy.Service(
            "execute_add_collision_box", ExecuteAddCollisionBox, self.add_box_handle
        )
        self._srv_add_plane = rospy.Service(
            "execute_add_collision_plane",
            ExecuteAddCollisionPlane,
            self.add_plane_handle,
        )
        self._srv_attach_box = rospy.Service(
            "execute_attach_collision_box",
            ExecuteAttachCollisionBox,
            self.attach_box_handle,
        )
        self._srv_detach_obj = rospy.Service(
            "execute_detach_collision",
            ExecuteDetachCollision,
            self.detach_object_handle,
        )
        self._srv_remove_obj = rospy.Service(
            "execute_remove_collision",
            ExecuteRemoveCollision,
            self.remove_object_handle,
        )

        # General utilities
        self._srv_get_prepare_pose = rospy.Service(
            "get_prepare_pose", GetPreparePose, self.prepare_pose_handle
        )
    ...
    def get_pose_handle(self, req):
        ...
        try:
            ...
            resp.ee_link, resp.ref_link = self.interface.get_frame_of_group(
                req.group_name
            )
            resp.result_status = resp.SUCCEEDED
        except IndexError:
    ...
    def group_position_handle(self, req):
        ...
        if req.goal_type == req.BASE_ABS:
            ok = self.interface.group_go_to_global_base_position(
                req.group_name, req.goal, req.tolerance
            )
        elif req.goal_type == req.BASE_REL:
            ok = False
        else:
            ok = self.interface.group_go_to_eef_position(
                req.group_name, req.goal, req.tolerance
            )
    ...
    def group_pose_handle(self, req):
        if not req.tolerance:
            req.tolerance = 0.01
        if req.goal_type == req.BASE_ABS:
            ok = self.interface.group_goto_pose_goal_abs(
                req.group_name, req.goal, req.tolerance, req.constraint
            )
        elif req.goal_type == req.BASE_REL:
            ok = self.interface.group_goto_pose_goal_rel(
                req.group_name, req.goal, req.tolerance, req.constraint
            )
        elif req.goal_type == req.EEF:
            ok = self.interface.group_goto_pose_goal_eef(
                req.group_name, req.goal, req.tolerance, req.constraint
            )
        else:
    ...
    def group_js_handle(self, req):
        if not req.tolerance:
            req.tolerance = 0.01
        ok = self.interface.group_go_to_joint_states(
            req.group_name, req.goal, req.tolerance
        )
    ...
    def group_plan_handle(self, req):
        if req.is_absolute:
            plan = self.interface.build_absolute_path_for_group(
    ...
    def all_poses_handle(self, req):
        if req.goal_type == req.BASE_ABS:
            ok = self.interface.all_goto_pose_goals_abs(
                req.group_names,
                req.goals,
                req.tolerance,
                req.stamps,
            ...
            ok = self.interface.all_goto_pose_goals_rel(
                req.group_names,
                req.goals,
                req.tolerance,
                req.stamps,
    ...
    def attach_box_handle(self, req):
        ok = self.interface.attach_box(
            req.group_name, req.eef_group_name, req.box_name, req.box_pose, req.box_size
        )
    ...
    def add_plane_handle(self, req):
        ok = self.interface.add_plane(
            req.group_name, req.plane_name, req.plane_pose, req.plane_normal
        )
    ...
    def transformed_pose_handle(self, req):
        ok, pose = self.interface.get_transformed_pose(
            req.pose, req.source_frame, req.target_frame
        )
