...
class HPPManipulationClient(object):
    ...
    def __init__(self, kwargs):
        ...
        self._srv_execute_path_planning = rospy.Service(
            "execute_path_planning",
            ExecutePathPlanning,
            self.execute_path_planning_handle,
        )

        self._srv_execute_manipulation_planning = rospy.Service(
            "execute_manipulation_planning",
        ...
        try:
            ...
            self._sub_joint_state = rospy.Subscriber(
                js_topic, JointState, self.js_update_cb, buff_size=1
            )
        except KeyError:
        ...
        try:
            ...
            self._sub_odom = rospy.Subscriber(
                odom_topic, Odometry, self.odom_update_cb, buff_size=1
            )
        except KeyError:
        ...
        try:
            ...
            self._sub_object_pose = rospy.Subscriber(
                object_pose_topic, Pose, self.object_pose_update_cb, buff_size=1
            )
        except KeyError:
    ...
    def execute_path_planning_handle(self, req):
        ...
        base_goal_pose = self._to_global_pose(
            req.base_goal_pose, req.base_goal_type, base_current_global_pose
        )
        ok = self.interface.approach(
            base_goal_pose,
            req.joint_goal_state,
            req.base_pos_tolerance,
    ...
    def execute_manipulation_planning_handle(self, req):
        ...
        base_current_global_pose = self.interface.get_current_base_global_pose()
        object_goal_pose = self._to_global_pose(
            req.object_goal_pose, req.object_goal_pose_type, base_current_global_pose
        )
        # base_goal_pose = self._to_global_pose(req.base_goal_pose, req.base_goal_pose_type, base_current_global_pose)
        ok = self.interface.grasp(
            req.joint_goal_state,
            object_goal_pose,
            req.object_pos_tolerance,
