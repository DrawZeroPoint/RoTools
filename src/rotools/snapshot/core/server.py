...
class SnapshotServer(object):
    ...
    def __init__(self, kwargs):
        ...
        kwargs["js_topics"] = self.register_topics(
            kwargs["js_topics"], sensor_msgs.msg.JointState
        )
        kwargs["odom_topics"] = self.register_topics(
            kwargs["odom_topics"], nav_msgs.msg.Odometry
        )
        kwargs["pose_topics"] = self.register_topics(
            kwargs["pose_topics"], geometry_msgs.msg.Pose
        )

        self.register_topics(
            kwargs["rgb_compressed_topics"], sensor_msgs.msg.CompressedImage
        )
        self.register_topics(
            kwargs["depth_compressed_topics"], sensor_msgs.msg.CompressedImage
        )

        self.interface = interface.SnapshotInterface(**kwargs)

        self._srv_save_joint_state = rospy.Service(
            "save_joint_state", SaveJointState, self.save_joint_state_handle
        )
        self._srv_save_odom = rospy.Service(
            "save_odom", SaveOdometry, self.save_odom_handle
        )
        self._srv_save_pose = rospy.Service(
            "save_pose", SavePose, self.save_pose_handle
        )
        self._srv_save_image = rospy.Service(
            "save_image", SaveImage, self.save_image_handle
        )
        ...
        if isinstance(topics, list) or isinstance(topics, tuple):
            ...
            for topic in topics:
                ...
                if real_type is None:
                    rospy.logwarn(
                        "Topic {} has not been published, type check skipped".format(
                            topic
                        )
                    )
                else:
                    ...
                    if type_name != real_type and type_name != real_type.split("/")[-1]:
                        rospy.logerr(
                            "Topic {} assigned as {}, but the real type is {}, omitted".format(
                                topic, type_name, real_type
                            )
                        )
                        continue
                subscriber = rospy.Subscriber(
                    topic, topic_type, self.msg_cb, topic, queue_size=1
                )
                self._subscribers.append(subscriber)
                rospy.loginfo(
                    "Registered topic {} of type {}".format(topic, topic_type.__name__)
                )
    ...
    def save_image_handle(self, req):
        ...
        ok = self.interface.save_image_msgs(
            req.rgb_topic, req.depth_topic, rgb_msg, depth_msg, req.tag
        )
    ...
    def save_joint_state_handle(self, req):
        ...
        if req.topic in self._msg_dict:
            ...
            ok = self.interface.save_joint_state_msg(
                req.topic, msg, req.position_only, req.tag
            )
            resp.result_status = resp.SUCCEEDED if ok else resp.FAILED
        else:
            rospy.logwarn(
                "Joint state topic {} has not been recorded".format(req.topic)
            )
